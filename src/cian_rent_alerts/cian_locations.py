...
CIAN_LOCATIONS: tuple[tuple[str, str], ...] = (
    ("Москва", "1"),
    ("Санкт-Петербург", "2"),
    ("Абакан", "4638"),
    ("Анадырь", "4648"),
...
CIAN_LOCATION_REGION_IDS = dict(CIAN_LOCATIONS)


def find_cian_location(value: str) -> tuple[str, str] | None:
    normalized = normalize_location_name(value)
    if not normalized:
        return None

    for city, region_id in CIAN_LOCATIONS:
        if normalize_location_name(city) == normalized:
